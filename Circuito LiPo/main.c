...
{
	int celula1 = 1024, celula2 = 708, celula3 = 392; // Não sei o que são esses valores iniciais, coisa da Bianca
   ...
   CMCONbits.CM = 0b111;	// Desliga módulo de comparador (desconecta pino RA2 do módulo)
	...
	ADCON0bits.ADON = 1;	// AD on: Liga o periférico de conversão
   ...
   ADCON0bits.CHS = 0b010;	// Channel select: Seleciona o canal 2 para conversão (RA2/AN2 - célula superior)
	...
	{    
	    ...
	    */     
		...
		PORTCbits.RC1 = !seguro;	// RC1 (transostor de saída) em nível baixo se for seguro, alto se não for.
		 ...
		 * - Inverter comportamento do transistor de saída (ver arquivo "Circuito LiPo - Comentários", seção Ideias)
		 */
    }
